...
utils::wait-for-condition 'federated deployment status updated in cluster1' \
    "(kubectl get federateddeployment nginx -n ${TEST_NS} -o jsonpath='{.status.clusters[*].name}' | grep '\<cluster1\>') &> /dev/null" 120
utils::wait-for-condition 'federated deployment status updated in cluster2' \
...
utils::wait-for-condition 'nginx web server ok in cluster1' "curl ${IP1}:${PORT1} &> /dev/null" 120
utils::wait-for-condition 'nginx web server ok in cluster2' "curl ${IP2}:${PORT2} &> /dev/null" 120
...
utils::wait-for-condition 'web content changed in cluster1' "(curl ${IP1}:${PORT1} | grep '^Hello from KubeFed') &> /dev/null" 120
utils::wait-for-condition 'web content changed in cluster2' "(curl ${IP2}:${PORT2} | grep '^Hello from KubeFed') &> /dev/null" 120
echo "cluster1: $(curl -s ${IP1}:${PORT1})"
echo "cluster2: $(curl -s ${IP2}:${PORT2})"
...
utils::wait-for-condition 'replicas updated in cluster2' \
...
utils::wait-for-condition 'federated deployment status updated' \
    "(kubectl get federateddeployment nginx -n ${TEST_NS} -o jsonpath='{.status.clusters[*].name}' | grep "^cluster2$") &> /dev/null" 120
