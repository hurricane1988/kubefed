...
ROOT_DIR="$(pwd)"
NAMESPACE="${KUBEFED_NAMESPACE:-kube-federation-system}"
...
for filename in ./config/enabletypedirectives/*.yaml; do
  ...
  ./bin/kubefedctl --kubeconfig "${HOME}"/.kube/config enable -f "${filename}" --kubefed-namespace="${NAMESPACE}" -o yaml > "${full_name}"
